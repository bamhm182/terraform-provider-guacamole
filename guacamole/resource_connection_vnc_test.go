...
func TestAccGuacamoleConnectionVNCBasic(t *testing.T) {
	testProviderConnectionVNC := map[string]interface{}{
		...
		"parameters": map[string]interface{}{
			...
			"enable_audio":               true,
			"audio_server_name":          "audio-server.example.com",
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr("guacamole_connection_vnc.new", "parameters.0.enable_audio", boolToString(testProviderConnectionVNC["parameters"].(map[string]interface{})["enable_audio"].(bool))),
					resource.TestCheckResourceAttr("guacamole_connection_vnc.new", "parameters.0.audio_server_name", testProviderConnectionVNC["parameters"].(map[string]interface{})["audio_server_name"].(string)),
